...
| Einfacher Launcher | `MECK_SIMPLE_LAUNCHER` | Neuer Home-Screen: Karussell mit 5 großen Icons (**E-Reader → Music → Audiobooks → MeshCore → Settings**). Wischen oder Tippen links/rechts blättert, Tippen in der Mitte (oder Enter) öffnet. **Music** spielt aus `/music` (ohne Lesezeichen), **Audiobooks** aus `/audiobooks` (mit Fortsetzen-Position) — beide teilen sich einen Player; App-Wechsel stoppt die laufende Wiedergabe. MeshCore-Icon zeigt ungelesene Nachrichten und öffnet direkt die Kanal-/Nachrichtenliste — das alte Meck-Home (Statusseiten) ist archiviert. **Hinweis:** Damit ist auch Standby/Power-Off derzeit nicht erreichbar (`UITask::shutdown()` existiert weiter, bei Bedarf als Settings-Eintrag nachrüstbar). |
| Einfache Settings | `MECK_SIMPLE_LAUNCHER` | Settings-Icon öffnet eine flache Liste: **WiFi / LoRa Mesh / GPS** als Direkt-Schalter (Tap kippt sofort), **WiFi Setup >>** (Netz-Scan + Passwort), **IP: DHCP\|x.x.x.x** (Inline-Editor; leer = DHCP, IP eintippen = statisch mit Gateway/DNS = x.x.x.1), **File Manager >>** und **Advanced >>** (die kompletten alten Meck-Settings). Footer zeigt SSID + IP bei WiFi-Verbindung. |
| Dateiserver immer online | `MECK_FILEMGR_STA` | Sobald WiFi verbunden ist, läuft der SD-Web-Dateiserver automatisch im Hintergrund (Browser → Footer-IP). LoRa pausiert nur während aktiver Up-/Downloads (geteilter SPI-Bus), nicht mehr für die ganze Sitzung. |
| Zentrale WiFi-Konfig | — | `/web/wifi.cfg`: Zeile 1 SSID, Zeile 2 Passwort, Zeile 3 optional statische IP (leer = DHCP). `ui-new/WifiConfig.h` ersetzt die zuvor 5× duplizierte Verbindungslogik. |
