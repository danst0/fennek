...
void DataStore::loadPrefsInt(const char *filename, NodePrefs& _prefs, double& node_lat, double& node_lon) {
  ...
  if (file) {
    ...
    if (file.read((uint8_t *)&_prefs.lora_enabled, sizeof(_prefs.lora_enabled)) != sizeof(_prefs.lora_enabled)) {
      _prefs.lora_enabled = 1;  // default: LoRa on
    }
    if (file.read((uint8_t *)&_prefs.wifi_autostart, sizeof(_prefs.wifi_autostart)) != sizeof(_prefs.wifi_autostart)) {
      _prefs.wifi_autostart = 1;  // default: upstream behaviour (WiFi on at boot)
    }
    ...
    if (_prefs.lora_enabled > 1) _prefs.lora_enabled = 1;
    if (_prefs.wifi_autostart > 1) _prefs.wifi_autostart = 1;
...
void DataStore::savePrefs(const NodePrefs& _prefs, double node_lat, double node_lon) {
  ...
  if (file) {
    ...
    file.write((uint8_t *)&_prefs.lora_enabled, sizeof(_prefs.lora_enabled));                    // 176
    file.write((uint8_t *)&_prefs.wifi_autostart, sizeof(_prefs.wifi_autostart));                // 177
