...
8. **NVS (Preferences) für Settings/Bookmarks** — interner Flash, nie der SPI-Bus. SD nur für Bulk-Caches (`/.fennek/tracks.bin`, `/.fennek/id3.bin`, `/.fennek/idx/`, `/books/.epub_cache/`). Mesh-Identity/Kontakte auf SPIFFS. Einmalige Migrationen vom Meck-Erbe laufen automatisch: NVS `meck`→`fennek` (settings.cpp) und SD `/.meck`→`/.fennek` (main.cpp).
...
- `core/console.*` — Serial-Debug-Konsole (USB, `help`): `status`, `mesh init`, `advert`, `public <text>`, `dm <idx> <text>`, `contacts`, `msgs`, `wifi ssid/pass/status/start/stop`. Sende-Befehle initialisieren das Mesh bei Bedarf automatisch — Mesh-Tests laufen damit komplett ohne Display; WLAN-Zugangsdaten gehen hier oder über die Settings-App.
